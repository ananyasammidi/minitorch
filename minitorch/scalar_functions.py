...
class ScalarFunction:
    ...
    def apply(cls, *vals: "ScalarLike") -> Scalar:
        """Apply function creates the context, calls forward the variables, and makes a new variable using the result while incorporating the new history"""
        raw_vals = []
        scalars = []
        for v in vals:
...
class Add(ScalarFunction):
    ...
    def forward(ctx: Context, a: float, b: float) -> float:
        """Passing context and arguments to the neural network"""
        return a + b

    @staticmethod
    def backward(ctx: Context, d_output: float) -> Tuple[float, ...]:
        """Calculates the gradient of the output using the inputs"""
